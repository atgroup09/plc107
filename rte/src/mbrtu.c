...
/** @brief  Set interface settings.
 *  @param  MBRTUIn     - pointer to interface.
 *  @param  SlaveIdIn   - slave ID.
 *  @param  SettingsIn  - settings.
 *  @param  AllowModeIn - allow change mode.
 *  @return none.
 */
void MBRTU_SetSettings(MBRTU_t *MBRTUIn, const uint8_t SlaveIdIn, const MBRTU_Set_t SettingsIn, const uint8_t AllowModeIn)
{
    if(MBRTUIn)
    {
        MBRTUIn->SlaveID = SlaveIdIn;
        MBRTUIn->Settings.BaudRate  = SettingsIn.BaudRate;
        MBRTUIn->Settings.DataBits  = MBRTU_DATA_BITS_8;
        MBRTUIn->Settings.StopBits  = SettingsIn.StopBits;
        MBRTUIn->Settings.Parity    = SettingsIn.Parity;
        if(AllowModeIn) MBRTUIn->Settings.Mode = SettingsIn.Mode;
        MBRTUIn->Settings.ByteOrder = SettingsIn.ByteOrder;

#ifdef DEBUG_LOG_MBRTU
        DebugLog(" MBRTU: SlaveID=%d baud=%d dbits=%d sbits=%d prty=%d mode=%d bord=%d [SET]\n", MBRTUIn->SlaveID, MBRTUIn->Settings.BaudRate, MBRTUIn->Settings.DataBits, MBRTUIn->Settings.StopBits, MBRTUIn->Settings.Parity, MBRTUIn->Settings.Mode, MBRTUIn->Settings.ByteOrder);
#endif // DEBUG_LOG_MBRTU
    }
}


/** @brief  Init. interface by default.
 *  @param  MBRTUIn  - pointer to interface.
 *  @param  UsartIn  - USART.
 *  @param  SlaveIn  - slave ID.
 *  @param  DePortIn - pointer to DE-Port.
 *  @param  DePinIn  - number of DE-Pin.
 *  @param  UsartInitFuncIn - pointer to function of UART-init.
 *  @return none.
 *  @note   115200 8 N 1
 */
void MBRTU_InitDef(MBRTU_t *MBRTUIn, USART_TypeDef *UsartIn, const uint8_t SlaveIn, GPIO_TypeDef *DePortIn, const uint16_t DePinIn, void *UsartInitFuncIn)
{
    if(MBRTUIn && UsartIn)
    {
        MBRTUIn->USARTx = UsartIn;

        MBRTUIn->DePort = DePortIn;
        MBRTUIn->DePin  = DePinIn;
        ...
        MBRTUIn->FuncInit    = UsartInitFuncIn;
    }
}

/** @brief  Init. Settings from REG.
 *  @param  MBRTUIn     - pointer to interface.
 *  @return none.
 */
void MBRTU_InitReg(MBRTU_t *MBRTUIn)
{
    if(MBRTUIn)
    {
        uint16_t SlaveID = PLC_MBRTU_SLAVE_ADDR;
        uint16_t SetPack = PLC_COM1_SET;
        MBRTU_Set_ut Set;

        //read from REG_t (retain)
        //REG_CopyRegByPos(REG_SYS_SET__POS_MBRTU_SLAVE_ADDR, REG_COPY_MB_TO_VAR, &SlaveID);
        //REG_CopyRegByPos(REG_SYS_SET__POS_COM1, REG_COPY_MB_TO_VAR, &SetPack);
        //unpack settings
        Set.Data = SetPack;
        //set settings
        MBRTU_SetSettings(MBRTUIn, (uint8_t)SlaveID, Set.Settings, BIT_TRUE);

#ifdef DEBUG_LOG_MBRTU
        DebugLog(" MBRTU: SlaveID=%d Set.Data=%d\n", SlaveID, SetPack);
#endif // DEBUG_LOG_MBRTU
...
/** @brief  Convert baudrate code to USART-defines.
 *  @param  BaudRateIn - baudrate code:
 *  @arg      = 0 .. 7
 *  @return USART-defined value of baudrate.
 */
uint32_t MBRTU_UsartDefBaudRate(const uint8_t BaudRateIn)
{
    switch(BaudRateIn)
    {
        case MBRTU_BAUDRATE_1200:
            return (uint32_t)(1200);
        case MBRTU_BAUDRATE_2400:
            return (uint32_t)(2400);
        case MBRTU_BAUDRATE_4800:
            return (uint32_t)(4800);
        case MBRTU_BAUDRATE_9600:
            return (uint32_t)(9600);
        case MBRTU_BAUDRATE_19200:
            return (uint32_t)(19200);
        case MBRTU_BAUDRATE_38400:
            return (uint32_t)(38400);
        case MBRTU_BAUDRATE_57600:
...
/** @brief  Convert data-bits code to USART-defines.
 *  @param  DataBitsIn - data-bits code:
 *  @arg      = 0 .. 1
 *  @return USART-defined value of data-bits.
 */
inline uint16_t MBRTU_UsartDefDataBits(const uint8_t DataBitsIn)
{
    return (uint16_t)((DataBitsIn == MBRTU_DATA_BITS_9) ? USART_WordLength_9b : USART_WordLength_8b);
}


/** @brief  Convert parity code to USART-defines.
 *  @param  ParityIn - parity code:
 *  @arg      = 0 .. 2
 *  @return USART-defined value of parity.
 */
uint16_t MBRTU_UsartDefParity(const uint8_t ParityIn)
{
    switch(ParityIn)
    {
        case MBRTU_PARITY_ODD:
            return (uint16_t)(USART_Parity_Odd);
        case MBRTU_PARITY_EVEN:
            return (uint16_t)(USART_Parity_Even);
    }
    return (uint16_t)(USART_Parity_No);
}


/** @brief  Convert stop-bits code to USART-defines.
 *  @param  StopBitsIn - stop-bits code:
 *  @arg      = 0 .. 1
 *  @return USART-defined value of stop-bits.
 */
inline uint16_t MBRTU_UsartDefStopBits(const uint8_t StopBitsIn)
{
    return (uint16_t)((StopBitsIn == MBRTU_STOP_BITS_2) ? USART_StopBits_2 : USART_StopBits_1);
}
 ...
 *  @return CRC16.
 */
uint16_t MBRTU_CalcCRC16(const uint8_t *FrameIn, const uint8_t FrameSzIn)
{
 ...
 */
inline uint8_t MBRTU_TestSlaveID(const uint8_t Slave1In, const uint8_t Slave2In)
{
 ...
 */
uint16_t MBRTU_TestTable(const uint8_t TableIn)
{
    switch(TableIn)
    {
 ...
 */
uint8_t MBRTU_TestFunc(const uint8_t FuncIn)
{
    switch(FuncIn)
    {
 ...
 */
uint8_t MBRTU_TestCRC16(const uint8_t *FrameIn, const uint8_t FrameSzIn, const uint8_t HiIn, const uint8_t LoIn)
{
 ...
 *             MBRTUIn->SlaveID
 */
uint8_t MBRTU_CreateResHeader(MBRTU_t *MBRTUIn, const uint8_t ExcModeIn)
{
    if(MBRTUIn)
    {
        //+ Slave ID
 ...
 *           ~ MBRTUIn->TxBuff (set)
 */
uint8_t MBRTU_CreateResExc(MBRTU_t *MBRTUIn, const uint8_t ExcIn)
{
    if(MBRTUIn)
    {
        MBRTUIn->TxCnt = 0;
 ...
 *           ~ MBRTUIn->RxExc  (set)
 */
uint8_t MBRTU_TestHoldReq(MBRTU_t *MBRTUIn, const uint8_t ReqTyIn, const uint8_t ModeIn)
{
    uint8_t ReqSz = 0;
 ...
 *             MBRTUIn->RxBuff
 */
uint8_t MBRTU_TestHoldNRegs(MBRTU_t *MBRTUIn, const uint8_t ReqTyIn)
{
 ...
 *             MBRTUIn->RxEndAddr
 */
uint8_t MBRTU_ReadRegs(MBRTU_t *MBRTUIn, const uint8_t TableIn)
{
    REG_t   *Reg;
    uint16_t iAddr;
    uint8_t  i, RegWsz;
 ...
 */
uint8_t MBRTU_WriteHoldRegs(MBRTU_t *MBRTUIn, const uint8_t ModeIn)
{
    REG_t   *Reg;
    uint16_t iAddr;
 ...
 *           ~ MBRTUIn->RxExc  (set)
 */
uint8_t MBRTU_TestCoilReq(MBRTU_t *MBRTUIn, const uint8_t ReqTyIn, const uint8_t ModeIn)
{
    uint8_t ReqSz = 0;
 ...
 *             MBRTUIn->RxBuff
 */
uint8_t MBRTU_TestCoilNRegs(MBRTU_t *MBRTUIn, const uint8_t ReqTyIn)
{
    uint16_t NRegsMin = 0, NRegsMax = 0;
 ...
 *             MBRTUIn->RxEndAddr
 */
uint8_t MBRTU_ReadDiscretes(MBRTU_t *MBRTUIn, const uint8_t TableIn)
{
    REG_t   *Reg;
    uint16_t iAddr;
    uint8_t  iBytes;
 ...
 *             MBRTUIn->RxNumBytes  (set)
 */
uint8_t MBRTU_WriteCoils(MBRTU_t *MBRTUIn, const uint8_t ModeIn)
{
    REG_t   *Reg;
    uint16_t iAddr;
    uint8_t  iRx;
 ...
 *             MBRTUIn->SubFunc
 */
uint8_t MBRTU_DiagCount(MBRTU_t *MBRTUIn, const uint16_t CntIn)
{
    if(MBRTUIn)
    {
        //Create response
 ...
 *  @return Exception code.
 */
uint8_t MBRTU_SendRes(MBRTU_t *MBRTUIn)
{
    if(MBRTUIn)
    {
        if(MBRTUIn->RxExc < MBRTU_EXC_SLAVE_ERR && MBRTUIn->TxCnt)
        {
            MBRTUIn->cRes++;
            MBRTU_Start(MBRTUIn, MBRTU_TX);
        }
        else
        {
            MBRTUIn->cNoRes++;
            MBRTU_Start(MBRTUIn, MBRTU_RX);
        }
    }
    return (MBRTU_EXC_NONRECOV_ERR);
}
...
/** @brief  Start Rx/Tx.
 *  @param  MBRTUIn - pointer MBRTU-context.
 *  @param  DirIn   - transmit direction:
 *  @arg      = MBRTU_RX
 *  @arg      = MBRTU_TX
 *  @return None.
 */
void MBRTU_Start(MBRTU_t *MBRTUIn, const uint8_t DirIn)
{
    if(MBRTUIn)
    {
        if(DirIn == MBRTU_RX)
        {
            //IRQ.Off
            USART_ITConfig(MBRTUIn->USARTx, USART_IT_TC, DISABLE);

            MBRTUIn->RxTx       = MBRTU_RX;
            MBRTUIn->RxExc      = MBRTU_EXC_OK;
            MBRTUIn->RxBuffOver = BIT_FALSE;
            MBRTUIn->RxCnt      = 0;
            //DE.Off
            if(MBRTUIn->DePort) GPIO_ResetBits(MBRTUIn->DePort, MBRTUIn->DePin);
        }
        else if(DirIn == MBRTU_TX)
        {
            MBRTUIn->RxTx = MBRTU_TX;
            MBRTUIn->cTx  = 1;
            //DE.On
            if(MBRTUIn->DePort) GPIO_SetBits(MBRTUIn->DePort, MBRTUIn->DePin);
            //send first byte manually (other over USART.IRQ.TC)
            USART_SendData(MBRTUIn->USARTx, (uint16_t)MBRTUIn->TxBuff[0]);
            //IRQ.On
            USART_ITConfig(MBRTUIn->USARTx, USART_IT_TC, ENABLE);
        }
    }
}
 ...
 *  @return None.
 */
void MBRTU_Stop(MBRTU_t *MBRTUIn)
{
    if(MBRTUIn)
    {
        //DE.Off
        if(MBRTUIn->DePort) GPIO_ResetBits(MBRTUIn->DePort, MBRTUIn->DePin);
        //IRQ.Off
        USART_ITConfig(MBRTUIn->USARTx, USART_IT_IDLE, DISABLE);
        USART_ITConfig(MBRTUIn->USARTx, USART_IT_RXNE, DISABLE);
        USART_ITConfig(MBRTUIn->USARTx, USART_IT_TXE, DISABLE);
        USART_ITConfig(MBRTUIn->USARTx, USART_IT_TC, DISABLE);
        //IRQ.clear
        (void)USART_GetFlagStatus(MBRTUIn->USARTx, USART_FLAG_IDLE);
        (void)USART_GetITStatus(MBRTUIn->USARTx, USART_IT_RXNE);
        USART_ClearITPendingBit(MBRTUIn->USARTx, USART_IT_RXNE);
        (void)USART_ReceiveData(MBRTUIn->USARTx);
        USART_ClearITPendingBit(MBRTUIn->USARTx, USART_IT_TC);
        //USART.Off
        USART_Cmd(MBRTUIn->USARTx, DISABLE);
    }
}
