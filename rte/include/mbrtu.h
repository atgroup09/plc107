...
/** @def COM1: slave ID (by default)
 */
#define MBRTU_COM1_SLAVE_ID         (uint8_t)PLC_MBRTU_SLAVE_ADDR

/** @def COM1: packed settings (by default)
 ...
 */
#define MBRTU_COM1_SETTINGS         (uint16_t)1031


/** @def RTOS: RX queue data
 */
#define RTOS_MBRTU_Q_RX_END       (uint8_t)1  //RX-End

/** @def RTOS: RX wait data-mutex (ticks)
 */
#define RTOS_MBRTU_M_RX_WAIT      (TickType_t)50


/** @var COM1
 */
extern MBRTU_t MBRTU_COM;
 ...
 *  @param  UartInitFuncIn - pointer to function of UART-init.
 *  @return none.
 *  @note   115200 8 N 1
 */
void MBRTU_InitDef(MBRTU_t *MBRTUIn, USART_TypeDef *UsartIn, const uint8_t SlaveIn, GPIO_TypeDef *DePortIn, const uint16_t DePinIn, void *UartInitFuncIn);
 ...
 */
uint16_t MBRTU_UsartDefDataBits(const uint8_t DataBitsIn);
 ...
 */
uint16_t MBRTU_UsartDefStopBits(const uint8_t StopBitsIn);
 ...
 */
uint8_t MBRTU_TestSlaveID(const uint8_t Slave1In, const uint8_t Slave2In);
